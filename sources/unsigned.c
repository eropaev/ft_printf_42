...
void	pst_uint_lspace(t_flagi *flag, int dlinna, uintmax_t nb, int type)
{
	...
	else if (flag->pres)
	{
		if (type == 1 || type == 2 || type == 4)
			zeroo(flag->pres, un_int_poluchit_dlin(nb, 16));
		if (type == 3)
			zeroo(flag->pres, un_int_poluchit_dlin(nb, 8));
		if (type == 0)
			zeroo(flag->pres, un_int_poluchit_dlin(nb, 10));
		if (type == 5)
			zeroo(flag->pres, un_int_poluchit_dlin(nb, 2));
	}
}
